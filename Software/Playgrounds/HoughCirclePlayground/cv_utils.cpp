...
namespace golf_sim {
    ...
    int CvUtils::CircleRadius(const GsCircle& circle)
    {
    ...
    cv::Mat CvUtils::ConvertNDArrayToMat(const nc::NdArray<nc::uint8>& ndarr) {

        int rows = ndarr.shape().rows;
        int cols = ndarr.shape().cols;

        cv::Mat mat = cv::Mat(rows, cols, CV_8UC3);
        memcpy(mat.data, ndarr.data(), sizeof(uchar) * rows * cols * 3 /*Channel*/);
